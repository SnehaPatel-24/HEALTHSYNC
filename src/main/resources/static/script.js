...
async function login() {
    ...
    try {
        ...
        errorEl.textContent = err.message.includes('403') || err.message.includes('401')
...
async function register() {
    ...
    try {
        ...
        // Auto-fill login and switch to login tab after 1.5 seconds
        setTimeout(() => {
...
async function loadPatients(page = 0) {
    ...
    try {
        ...
        if (!rows.length) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#6b7280;padding:30px">No patients found. Add one!</td></tr>';
            return;
        }
...
function prevPage(module) {
    if (module === 'patients' && pagination.patients.page > 0)
        loadPatients(pagination.patients.page - 1);
}
function nextPage(module) {
    if (module === 'patients' && pagination.patients.page < pagination.patients.totalPages - 1)
        loadPatients(pagination.patients.page + 1);
}
...
async function savePatient() {
    ...
    if (!firstName || !lastName || !gender) {
        errorEl.textContent = 'First name, last name and gender are required.';
        return;
    }
    ...
    try {
        if (id) {
            await api(`/api/patients/${id}`, 'PUT', body);
...
async function saveDoctor() {
    ...
    try {
        if (id) {
            await api(`/api/doctors/${id}`, 'PUT', body);
            showToast('Doctor updated successfully!');
        } else {
            await api('/api/doctors', 'POST', body);
...
async function completeAppointment(id) {
    try {
        await api(`/api/appointments/${id}/complete`, 'PUT');
...
async function savePrescription() {
    ...
    const patientId = document.getElementById('pr-patientId').value;
    const doctorId  = document.getElementById('pr-doctorId').value;
    ...
    if (!patientId || !doctorId || !diagnosis || !medicines) {
        errorEl.textContent = 'Patient ID, Doctor ID, diagnosis and medicines are required.';
        return;
    }

    const body = {
        patientId: parseInt(patientId), doctorId: parseInt(doctorId),
        appointmentId: document.getElementById('pr-appointmentId').value
            ? parseInt(document.getElementById('pr-appointmentId').value) : null,
        diagnosis, medicines,
    ...
    try {
        if (id) {
            await api(`/api/prescriptions/${id}`, 'PUT', body);
            showToast('Prescription updated!');
        } else {
            await api('/api/prescriptions', 'POST', body);
